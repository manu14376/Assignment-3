...
class MatchGame extends Component {
  ...
  gameRestart = () => {
    this.setState({gameTime: 60, score: 0})
  }

  render() {
    const {activeTabId, imgList, gameTime, randomImage, score} = this.state
    const filterItems = imgList.filter(each => each.category === activeTabId)
    return (
      <div className="matchgame-container">
        <ul className="navbar">
          <Navbar gameTime={gameTime} score={score} />
        </ul>
        {gameTime !== 0 && (
        ...
        {gameTime === 0 && (
          <GameReset score={score} gameRestart={this.gameRestart} />
        )}
      </div>
    )
  }
